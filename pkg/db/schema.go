...
const (
	...
	insertMigrationSQL = `INSERT INTO migrations (id, name, version) VALUES ($1, $2, $3);`
)
...
func InitializeSchema() (err error) {
	lastApplied := -1
	// Attempt to fetch the latest migration applied to the database
	if err = conn.QueryRow(lastAppliedSQL).Scan(&lastApplied); err != nil {
		log.Warn().Err(err).Msg("could not fetch last applied migration")
	}
